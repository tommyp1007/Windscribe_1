...
class PreferencesMainViewController: PreferredFocusedViewController {
    ...
    private func bindViews() {
        ...
        accountViewModel.cancelAccountState
        ...
        accountViewModel.sessionUpdatedTrigger
            .sink { [weak self] _ in
                guard let self = self else { return }
    ...
    private func handleCancelAccount() {
        Task {
            ...
            if let password = await viewModel.alertManager.askPasswordToDeleteAccount(viewController: self),
               !password.isEmpty {
