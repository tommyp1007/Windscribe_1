...
class NotificationRepositoryTests: XCTestCase {
    ...
    // MARK: Read notices Tests

    func test_readNotice() async {
        do {
            _ = try await repository.getUpdatedNotifications()

            XCTAssertEqual(repository.readNotices.value.count, 0)

            repository.readNotice(with: 1)

            XCTAssertEqual(repository.readNotices.value.count, 1)
        } catch {
            XCTFail("Expected success, but got error: \(error)")
        }
    }

    func test_readNotice_multiple_times() async {
        do {
            _ = try await repository.getUpdatedNotifications()
            ...
            repository.readNotice(with: 2)

            XCTAssertEqual(repository.readNotices.value.count, 2)
        } catch {
            XCTFail("Expected success, but got error: \(error)")
        }
    }
    ...
    func test_notices_shouldEmitWhenDatabaseChanges() async {
        ...
        let noticesArray = Array(noticeList.notices).map { $0.getModel() }

        // Append one more to make it 2
        let secondNotice = NoticeModel(
            id: 2,
    ...
    func test_notices_shouldReactToMultipleUpdates() async {
        ...
        // First update
        mockLocalDatabase.notificationsSubject.send([NoticeModel(id: 0, title: "", message: "", date: 0, popup: false, action: nil)])

        // Second update
        let notice1 = NoticeModel(id: 1, title: "", message: "", date: 0, popup: false, action: nil)
        let notice2 = NoticeModel(id: 2, title: "", message: "", date: 0, popup: false, action: nil)
