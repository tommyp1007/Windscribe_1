...
class WSUIViewController: UIViewController {
    ...
    func openLink(url: String, asSheet: Bool = false) {
        guard let urlValue = URL(string: url) else { return }
        openLink(url: urlValue, asSheet: asSheet)
    }

    func openLink(url: URL, asSheet: Bool = false) {
    ...
    func showLoading() {
        DispatchQueue.main.async { [weak self] in
            ...
            self.loadingBackgroundView = UIView(frame: viewBounds)
            ...
            self.backgroundLoadingIndicator.frame = CGRect(x: viewBounds.width / 2 - 25, y: viewBounds.height / 2 - 25, width: 50, height: 50)
...
extension WSUIViewController: SFSafariViewControllerDelegate {
    func safariViewControllerDidFinish(_ controller: SFSafariViewController) {
        ...
        Task { try? await sessionManager.updateSession() }
    }
}
