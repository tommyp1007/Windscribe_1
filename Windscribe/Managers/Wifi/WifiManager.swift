...
// MARK: - WifiManager Protocol

protocol WifiManager {
    var selectedPreferredProtocol: String? { get }
    var selectedPreferredPort: String? { get }
    var selectedPreferredProtocolStatus: Bool? { get }
...
class WifiManagerImpl: WifiManager {
    ...
    private lazy var portMapRepository = Assembler.resolve(PortMapRepository.self)
    ...
    func saveCurrentWifiNetworks() {
        connectivity.network
            ...
            .sink(receiveCompletion: { [weak self] completion in
                ...
                if let suggestedPorts = self.portMapRepository.suggestedPorts,
    ...
    private func saveNewNetwork(wifiSSID: String) {
        let defaultProtocol = VPNProtocolType.wireGuard.identifier
        let defaultPort = portMapRepository.getPorts(protocolType: defaultProtocol)?.first ?? "443"
